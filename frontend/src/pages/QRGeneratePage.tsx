      ...
      >
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by code, product name, manufacturer, batch, or location..."
            className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-rail-blue/50 focus:ring-1 focus:ring-rail-blue/20 transition-all"
          />
          {searching && (
          ...
          {showDropdown && searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-10 overflow-hidden max-h-80 overflow-y-auto">
              {searchResults.map((f) => (
                <button
                  key={f.id}
                  onClick={() => {
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${
                          ...
                          f.status === 'UNDER_MAINTENANCE' ? 'bg-purple-100 text-purple-700' :
                      ...
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-slate-500">
                        {f.fitting_type_name && (
                          <span className="flex items-center gap-1">
                            <span className="text-slate-400">Type:</span> {f.fitting_type_name}
                          </span>
                        )}
                        {f.vendor_name && (
                          <span className="flex items-center gap-1">
                            <span className="text-slate-400">Vendor:</span> {f.vendor_name}
                          </span>
                        )}
                        {f.batch_number && (
                          <span className="flex items-center gap-1">
                            <span className="text-slate-400">Batch:</span> {f.batch_number}
                          </span>
                        )}
                        {f.location_name && (
                          <span className="flex items-center gap-1">
                            <span className="text-slate-400">Location:</span> {f.location_name}
                          </span>
                        )}
                      </div>
                    </div>
          ...
          {showDropdown && searchQuery && !searching && searchResults.length === 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-10 p-4 text-center">
              <p className="text-sm text-slate-500">No fittings found matching "{searchQuery}"</p>
              <p className="text-[11px] text-slate-400 mt-1">Try searching by code, product name, manufacturer, or batch number</p>
            </div>
          )}
        </div>
